...
$(KERNEL_CONFIG): kernel/arch/$(ARCH_)/configs/$(KERNEL_DEFCONFIG)
	...
	mkdir -p $(KERNEL_OUT)
	$(MAKE) ARCH=$(ARCH_) CONFIG_DEBUG_SECTION_MISMATCH=y -C kernel O=../$(KERNEL_OUT) $(KERNEL_DEFCONFIG)
...
$(TARGET_PREBUILT_KERNEL) : $(KERNEL_OUT) $(USER_CONFIG)  | $(KERNEL_CONFIG)
	$(MAKE) CONFIG_DEBUG_SECTION_MISMATCH=y -C kernel O=../$(KERNEL_OUT) ARCH=$(ARCH_) CROSS_COMPILE=$(CROSS_COMPILE_) headers_install
	$(MAKE) CONFIG_DEBUG_SECTION_MISMATCH=y -C kernel O=../$(KERNEL_OUT) ARCH=$(ARCH_) CROSS_COMPILE=$(CROSS_COMPILE_) -j${JOBS}
	$(MAKE) CONFIG_DEBUG_SECTION_MISMATCH=y -C kernel O=../$(KERNEL_OUT) ARCH=$(ARCH_) CROSS_COMPILE=$(CROSS_COMPILE_) modules
...
kernelheader:
	mkdir -p $(KERNEL_OUT)
	$(MAKE) ARCH=$(ARCH_) CONFIG_DEBUG_SECTION_MISMATCH=y -C kernel O=../$(KERNEL_OUT) headers_install
